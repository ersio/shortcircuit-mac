...
def dict_from_csvqfile(file_path):
    ...
    if qfile.open(QtCore.QIODevice.ReadOnly | QtCore.QIODevice.Text):
        text = qfile.readAll()
        f = StringIO(text.data().decode('utf8'))
        reader = csv.reader(f, delimiter=';')

    return reader


class CrestDialog(QtWidgets.QDialog, Ui_CrestDialog):
    """
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        wh_codes = {rows[0]: int(rows[1]) for rows in dict_from_csvqfile("../resources/database/statics.csv")}
        self.nav = Navigation(
            gates,
            system_desc,
            wh_codes,
    ...
    def btn_eve_login_clicked(self):
        if not self.eve_connected:
            ...
            qurl = QtCore.QByteArray(url.encode()) 
            QtGui.QDesktopServices.openUrl(QtCore.QUrl.fromEncoded(qurl))
        else:
            self.crestp.logout()
